...
const FilterMenu = ({showFilter,filterMenuProp}) => {
    ...
    return (
        <div className="d-flex align-items-center content justify-content-between filter_menu">
            <div className="menu">
                <ul>
                    {
                        (filterMenuProp ? filterMenuProp : filterMenu).map((item) => {
                            return(
            ...
            {
                showFilter === false ? '' : <div onClick={() => setFilterOption(!filterOption)} className={`advanced_filter d-flex align-items-center gap-2 pointer-event ${filterOption ? 'active':''}`}>
                <SwitchButton/>
                <p>Advanced mode</p>
            </div> 
            }
